...
const gtpCmsPages = [
  ...
  },
  {
    ...
    title: "Programme",
    notes:
      "Singleton `gtp2026Programme` (fixed id): days, sessions (venue type, venue line, format label, optional Objective, optional **What's On carousel background** image per session), workshops (optional Objective, Speakers, and Facilitators on each row). **Session and workshop popups — Hosted by:** heading, organisation logo (alt required with image), organisation name, location, and **Show location** (turn off to hide the location line only). Sessions and workshops each have a Facilitators list in the popup. Leave it empty to show “To be confirmed”. People already tagged with the role Facilitator under Speakers are shown there too, so they are not listed twice. For parallel sessions, add facilitators on each workshop row. Objective copy appears on the full programme page (session cards, concurrent tracks, workshop sub-cards, session detail modal) when filled—not on the small About hero carousel. Carousel card backgrounds: upload an image on a session to replace the default colour gradient; remove the image to revert. Drives programme page and parts of About.",
  },
  {
