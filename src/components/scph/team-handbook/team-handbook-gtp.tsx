...
const gtpCmsPages = [
  ...
  },
  {
    ...
    title: "Programme",
    notes:
      "Singleton `gtp2026Programme` (fixed id): days, sessions (**Closed event** checkbox for invitation-only items that keep their session type and also appear under the Closed Event filter, venue type, venue line, format label, optional Objective, optional **What's On carousel background** image per session), workshops (optional Objective, Speakers, and Facilitators on each row). **Hosted by:** the programme document sets the default heading, logo, organisation, location, and **Show location**. Each session and each workshop can override the organisation, logo, and location in its own **Hosted by** fields; leave those empty to keep the programme default. Sessions and workshops each have a Facilitators list in the popup. Leave it empty to show “To be confirmed”. People already tagged with the role Facilitator under Speakers are shown there too, so they are not listed twice. For parallel sessions, add facilitators on each workshop row. Objective copy appears on the full programme page (session cards, concurrent tracks, workshop sub-cards, session detail modal) when filled—not on the small About hero carousel. Carousel card backgrounds: upload an image on a session to replace the default colour gradient; remove the image to revert. Drives programme page and parts of About.",
  },
  {
