...
- **GTP About only**: `npm run seed-gtp-about-page` — upserts **`gtp2026AboutPage` only** with the same full-band payload as the combined seed (builder: [`scripts/lib/gtp-about-page-seed-doc.ts`](scripts/lib/gtp-about-page-seed-doc.ts)). Each band has **Visible on site** in Studio (like SCPH section blocks). **Partners:** uploads **PIK** into `partnersBand.partners` when the file exists; **Sponsors** band starts with an empty logo list. Re-running replaces the whole About document. After schema changes, `cd studio && npx sanity schema deploy`, then re-seed and **Publish**. `DRY_RUN=1` prints JSON only.
  - **Sponsors / partners tiers (Studio, no seed required):** On **GTP 2026 About page** → **Sponsors**, set each logo’s **Tier** (Platinum–Others); empty tiers show **Coming soon** on `/events/gtp-2026/about`. On **Partners**, use **Partner logos** for the main strip and **Supported by logos** for the secondary strip below. Add rows in Studio and **Publish**; do not re-run About seeds on production if editors already manage logos there.
- **GTP Media + Business forum + About**: `npm run seed-gtp-media-bizforum-pages` — upserts `gtp2026MediaPage` / `gtp2026BizForumPage` (placeholder shell) and `gtp2026AboutPage` (same About payload as `seed-gtp-about-page`). **Partners:** seed uploads **PIK** when the file exists. The hero **programme carousel** still uses `gtp2026Programme`. After schema changes, run `cd studio && npx sanity schema deploy`, re-seed, and **Publish**. `DRY_RUN=1` prints JSON only.
- **GTP Programme activity pages**: `SANITY_DATASET=development npm run seed-gtp-programme-subpages` — creates missing Action Workshops, AI Thinkers Networking Breakfast, Film Screening, and Sensorial Station page documents without overwriting editor content. The script intentionally refuses the `production` dataset. Run `DRY_RUN=1` first, deploy the Studio schema, then publish from the test Studio when ready.
