...
- **GTP Programme activity pages**: `SANITY_DATASET=development npm run seed-gtp-programme-subpages` — creates missing Action Workshops, AI Thinkers Networking Breakfast, and Film Screening page documents without overwriting editor content. The script intentionally refuses the `production` dataset. Run `DRY_RUN=1` first, deploy the Studio schema, then publish from the test Studio when ready.
